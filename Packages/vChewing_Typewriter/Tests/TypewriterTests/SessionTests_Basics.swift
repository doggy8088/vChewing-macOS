...
extension InputHandlerTests {
  ...
  /// （`UserDefaults.unitTests`、`UserDef`、`LMAssembly` 的測試辭典、`SessionHost.shared`）。
  ...
  final class Session {
    ...
    init() {
      UserDefaults.unitTests = .init(suiteName: "org.atelierInmu.vChewing.Typewriter.SessionTests")
      ...
      session.inputHandler?.currentLM = lm
    ...
    deinit {
      ...
      // 不同 Suite 之間仍會並行；若在此重設 LMAssembly 共用狀態、UserDefaults 或
    ...
    let testLM: LMAssembly.LMInstantiator
